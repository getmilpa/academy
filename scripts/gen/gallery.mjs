...
import { renderRuntimeFallback } from "./runtime.mjs";
...
function itemName(a, lang) {
  if (a.id === "atomo") return ATOMO.title[lang];
  if (a.id === "runtime") return RUNTIME.title[lang];
  return L(a.title, lang);
}
function itemDescription(a, lang) {
  if (a.id === "atomo") return stripTags(ATOMO.lede[lang]);
  if (a.id === "runtime") return stripTags(RUNTIME.lede[lang]);
  return stripTags(L(a.lede, lang));
}
...
/* ---- Artifact 05 · runtime (fallback estático desde RUNTIME, P2c) ------- */

function renderRuntimeSection(a, lang) {
