(function () {
  ...
  function renderLesson(route) {
    ...
    var body = unit.understand.map(function (paragraph) { return "<p>" + escapeHtml(paragraph) + "</p>"; }).join("");
    var objectives = unit.objectives.map(function (objective) { return "<li>" + escapeHtml(objective) + "</li>"; }).join("");
    var rubric = unit.verify.map(function (criterion) { return "<li>" + escapeHtml(criterion) + "</li>"; }).join("");
    var sources = unit.sources.map(function (source) { return "<li><a href=\"" + escapeHtml(source.href) + "\">" + escapeHtml(source.label) + "</a></li>"; }).join("");
    var toc = tocMarkup();
    ...
    var doMarkup = "<section class=\"ac-phase\" id=\"hacer\"><h2><span class=\"ac-phase-index\" aria-hidden=\"true\">3</span>Hacer</h2><p>Ejecuta la práctica en tu checkout y conserva la salida como evidencia.</p>" + terminalMarkup(unit.do.commands) + "<div class=\"ac-action-row mui-not-prose\"><a class=\"mui-btn mui-btn--secondary\" href=\"" + escapeHtml(unit.do.href) + "\">" + escapeHtml(unit.do.label) + "</a></div></section>";
