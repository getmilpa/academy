    ...
    },
    {
      ...
      units: [
        ...
        },
        {
          ...
          see: { label: { es: "Abrir la compuerta del arranque", en: "Open the boot gate" }, href: "../artifacts/#compuerta-arranque", note: { es: "Recorre los cuatro escenarios del resolver: cada uno rinde el ResolutionReport real — la compuerta, el error aprendible y el boot en loadOrder[].", en: "Walk the resolver's four scenarios: each renders the real ResolutionReport — the gate, the learnable error and the loadOrder[] boot." } },
        ...
        {
          id: "admision-en-el-edge",
          title: { es: "La admisión vive donde ve la cola", en: "Admission lives where it sees the queue" },
          durationMinutes: 25,
          objectives: [
            { es: "Separar dos funciones que se confunden en 'admission control': el backpressure al cliente y la protección del recurso escaso.", en: "Separate two functions conflated in 'admission control': client backpressure and scarce-resource protection." },
            { es: "Ubicar por qué un gate dentro de la aplicación no puede rechazar la sobrecarga cuando la cola se forma antes, en el servidor.", en: "Locate why an in-application gate can't shed overload when the queue forms earlier, at the server." }
          ],
          understand: [
            { es: "Un límite de capacidad hace dos cosas distintas que es fácil fundir en una. La primera es backpressure: bajo sobrecarga, decirle al cliente 'ahora no, reintenta' con un 503/429 y un Retry-After, en vez de encolar sin cota. La segunda es proteger un recurso escaso —típicamente el pool de conexiones a la base de datos— acotando cuántas peticiones lo tocan a la vez. Son funciones distintas: la primera da una señal al cliente; la segunda evita que un pico ya admitido reviente el motor. Un gate de admisión dentro de la aplicación hace bien la segunda, y es tentador creer que por eso hace la primera. No la hace.", en: "A capacity limit does two different things that are easy to fuse into one. The first is backpressure: under overload, telling the client 'not now, retry' with a 503/429 and a Retry-After, instead of queueing without bound. The second is protecting a scarce resource —typically the database connection pool— by capping how many requests touch it at once. They are different functions: the first gives the client a signal; the second keeps an already-admitted spike from crushing the engine. An admission gate inside the application does the second well, and it's tempting to believe it therefore does the first. It doesn't." },
            { es: "El motivo es dónde se forma la cola. En un despliegue con pool de workers (por ejemplo el modo worker del runtime, con un número fijo de procesos long-lived), cuando la carga ofrecida supera la capacidad las peticiones se encolan UPSTREAM —en el servidor, esperando un worker libre— antes de entrar a la aplicación. El gate vive DENTRO del worker, downstream de esa cola: solo ve la petición cuando un worker ya la tomó, así que nunca ve el exceso encolado y no lo puede rechazar. El síntoma es inconfundible: bajo overload el gate registra cero rechazos mientras la latencia observada trepa a segundos. El backpressure solo lo puede dar quien ve la cola, y la cola vive en el edge —un reverse-proxy que rechace rápido (503/429 sin encolar) o el propio límite de concurrencia del pool de workers. La regla: la admisión que da backpressure pertenece al edge; el gate de la aplicación se queda para proteger la base de datos de lo que ya pasó el edge.", en: "The reason is where the queue forms. In a worker-pool deployment (for instance the runtime's worker mode, with a fixed number of long-lived processes), when offered load exceeds capacity the requests queue UPSTREAM —at the server, waiting for a free worker— before entering the application. The gate lives INSIDE the worker, downstream of that queue: it only sees a request once a worker has already picked it up, so it never sees the queued excess and can't reject it. The symptom is unmistakable: under overload the gate records zero rejections while observed latency climbs into seconds. Backpressure can only come from whoever sees the queue, and the queue lives at the edge —a reverse-proxy that rejects fast (503/429 without queueing) or the worker pool's own concurrency limit. The rule: admission that provides backpressure belongs at the edge; the application gate stays to protect the database from what already passed the edge." }
          ],
          see: { label: { es: "Ver la frontera del sistema", en: "See the system boundary" }, href: "../artifacts/#frontera", note: { es: "La frontera es donde el sistema decide qué entra; el backpressure de capacidad es una decisión de frontera más —vive donde se ve la cola, no en el fondo de la aplicación.", en: "The boundary is where the system decides what enters; capacity backpressure is one more boundary decision —it lives where the queue is seen, not deep inside the application." } },
          do: { label: { es: "Separar las dos capas en tu despliegue", en: "Separate the two layers in your deployment" }, href: "https://frankenphp.dev/docs/worker/", commands: [] },
          verify: [
            { es: "Distingue el backpressure (señal al cliente) de la protección del pool de conexiones (evitar que un pico reviente el motor), y di cuál da cada capa.", en: "Tell backpressure (a signal to the client) apart from connection-pool protection (keeping a spike from crushing the engine), and say which layer provides each." },
            { es: "Explica por qué un gate dentro de un worker registra cero rechazos bajo overload aunque la latencia trepe a segundos.", en: "Explain why a gate inside a worker records zero rejections under overload even as latency climbs into seconds." }
          ],
          sources: [
            { label: { es: "FrankenPHP — worker mode", en: "FrankenPHP — worker mode" }, href: "https://frankenphp.dev/docs/worker/" },
            { label: { es: "nginx — limit_conn (admisión por concurrencia)", en: "nginx — limit_conn (concurrency admission)" }, href: "https://nginx.org/en/docs/http/ngx_http_limit_conn_module.html" }
          ],
          lastVerified: "2026-07-23"
        },
        {
          id: "evidencia-bajo-carga",
          title: { es: "Los experimentos revelan, no confirman", en: "Experiments reveal, they don't confirm" },
          durationMinutes: 25,
          objectives: [
            { es: "Reconocer por qué una máquina de capacidad con tests unitarios verdes puede fallar bajo carga sostenida.", en: "Recognize why a capacity machine with green unit tests can still fail under sustained load." },
            { es: "Diseñar la validación para revelar el comportamiento que la hipótesis no explica, no para confirmarla.", en: "Design validation to reveal the behavior the hypothesis doesn't explain, not to confirm it." }
          ],
          understand: [
            { es: "Un gate de admisión puede tener su lógica probada en unidad —rechazar sobre el límite, liberar el slot, respetar la prioridad— y aun así fallar por tres razones que ninguna prueba unitaria puede ver. Un contador de ocupación con TTL de seguridad se resetea a mitad de vuelo si la carga se sostiene más que el TTL, y readmite de más: solo aparece pasado ese umbral de tiempo, nunca en una corrida corta. Un gate colocado después de la primera consulta a la base de datos no protege nada: la carga ya tocó el recurso antes de que el gate opine. Y en un modelo shared-nothing, abrir una conexión por petición agota los puertos efímeros a suficiente tasa. Los tres son invisibles al test de lógica y al benchmark de menos de 30 s; los tres se vuelven obvios con carga real y sostenida.", en: "An admission gate can have its logic unit-tested —reject over the limit, release the slot, respect priority— and still fail for three reasons no unit test can see. An occupancy counter with a safety TTL resets mid-flight if load is sustained longer than the TTL, and over-admits: it only shows up past that time threshold, never in a short run. A gate placed after the first database query protects nothing: the load already touched the resource before the gate weighs in. And in a shared-nothing model, opening a connection per request exhausts the ephemeral ports at enough throughput. All three are invisible to the logic test and to the sub-30s benchmark; all three become obvious under real, sustained load." },
            { es: "La lección no es 'haz más tests'; es para qué existe un experimento. No está para CONFIRMAR la hipótesis —'la máquina funciona, el benchmark lo prueba'— sino para REVELAR el comportamiento que la hipótesis todavía no explica. La herramienta de validación más valiosa es la capaz de observar un fenómeno que el resto de las pruebas no puede: el test unitario mide la lógica; el benchmark sostenido mide lo que emerge del tiempo, la concurrencia y las conexiones reales. Cuando el resultado refuta la hipótesis cómoda, ese es el valor, no el fracaso. De ahí la disciplina de evidencia: 'hecho' no es una afirmación, es un artefacto reproducible bajo las condiciones donde el sistema realmente corre —sostener la carga por encima de cualquier TTL, medir dónde se forma la cola, contar las conexiones reales.", en: "The lesson isn't 'write more tests'; it's what an experiment is for. It doesn't exist to CONFIRM the hypothesis —'the machine works, the benchmark proves it'— but to REVEAL the behavior the hypothesis doesn't yet explain. The most valuable validation tool is the one able to observe a phenomenon the rest of the tests can't: the unit test measures logic; the sustained benchmark measures what emerges from time, concurrency and real connections. When the result refutes the comfortable hypothesis, that is the value, not the failure. Hence the evidence discipline: 'done' isn't a claim, it's an artifact reproducible under the conditions where the system actually runs —sustaining load beyond any TTL, measuring where the queue forms, counting the real connections." }
          ],
          see: { label: { es: "Abrir la radiografía del runtime", en: "Open the runtime x-ray" }, href: "../artifacts/#runtime", note: { es: "La radiografía muestra que cada camino de una operación deja un rastro distinto —la misma disciplina de mirar el comportamiento real, paso a paso, que un benchmark sostenido aplica a la capacidad.", en: "The x-ray shows that each path of an operation leaves a distinct trace —the same discipline of looking at real behavior, step by step, that a sustained benchmark applies to capacity." } },
          do: { label: { es: "Sostener la carga por encima del TTL y medir dónde se forma la cola", en: "Sustain load beyond the TTL and measure where the queue forms" }, href: "https://k6.io/docs/", commands: [] },
          verify: [
            { es: "Da un ejemplo de un defecto de capacidad que un test unitario con lógica verde no puede exhibir, y di qué condición del benchmark lo revela.", en: "Give an example of a capacity defect a unit test with green logic can't exhibit, and say which benchmark condition reveals it." },
            { es: "Explica la diferencia entre un experimento que confirma la hipótesis y uno que revela lo que la hipótesis no explica.", en: "Explain the difference between an experiment that confirms the hypothesis and one that reveals what the hypothesis doesn't explain." }
          ],
          sources: [
            { label: { es: "k6 — pruebas de carga", en: "k6 — load testing" }, href: "https://k6.io/docs/" },
