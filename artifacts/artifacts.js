...
const boundaryNodes = {
  ...
  runtime: { kind: KIND.motor, title: "runtime", copy: { es: "Compone core, command, container, events, HTTP y resolver; el gate de arquitectura corre antes de bootear en loadOrder.", en: "Composes core, command, container, events, HTTP and resolver; the architecture gate runs before booting in loadOrder." }, role: { es: "kernel y bootstrap", en: "kernel and bootstrap" }, deps: "core · command · container · events · http · resolver", source: "getmilpa-runtime/composer.json" },
  resolver: { kind: KIND.motor, title: "resolver", copy: { es: "Resuelve el grafo de arquitectura: gate pre-boot, loadOrder[], errores aprendibles y MigrationPlan.", en: "Resolves the architecture graph: pre-boot gate, loadOrder[], learnable errors and MigrationPlan." }, role: { es: "resolución de contratos", en: "contract resolution" }, deps: "core · composer/semver", source: "getmilpa-resolver/src/Engine/GraphResolver.php" },
  ...
  plugin: { kind: KIND.contrato, title: "plugin", copy: { es: "Manifests de capabilities (provides/requires); la resolución y el orden de Kahn migraron a milpa/resolver.", en: "Capability manifests (provides/requires); resolution and Kahn ordering moved to milpa/resolver." }, role: { es: "modularidad", en: "modularity" }, deps: "core", source: "plugin/src/PluginManifest.php" },
  command: { kind: KIND.contrato, title: "command", copy: { es: "Operation como átomo agnóstico de superficie; CommandProvider y SurfaceProjector la proyectan a N superficies.", en: "Operation as the surface-agnostic atom; CommandProvider and SurfaceProjector project it to N surfaces." }, role: { es: "comando-como-átomo", en: "command-as-atom" }, deps: { es: "cero paquetes", en: "zero packages" }, source: "getmilpa-command/src/Operation.php" },
  "ai-gateway": { kind: KIND.adaptador, title: "ai-gateway", copy: { es: "Gateway LLM de dos proveedores con loop agéntico de tool-use; expone el ToolRegistry sin conocer el HTTP del host.", en: "Dual-provider LLM gateway with an agentic tool-use loop; exposes the ToolRegistry without knowing the host's HTTP." }, role: { es: "puerto LLM", en: "LLM port" }, deps: "core · tool-runtime · guzzle", source: "getmilpa-ai-gateway/composer.json" },
...
const boundaryFlows = {
  boot: ["host", "runtime", "resolver", "plugin", "core"],
